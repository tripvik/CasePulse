...
// Audio settings
static constexpr size_t record_size = 2048; // Reduced from 5000
...
static int16_t **rec_buffers = nullptr;
...
void setup()
{
  ...
  rec_buffers = (int16_t **)heap_caps_malloc(NUM_BUFFERS * sizeof(int16_t *), MALLOC_CAP_SPIRAM);
  if (rec_buffers)
  {
    M5.Log(ESP_LOG_VERBOSE, "Using PSRAM for buffer pointers");
    for (int i = 0; i < NUM_BUFFERS; ++i)
    {
      rec_buffers[i] = (int16_t *)heap_caps_malloc(record_size * sizeof(int16_t), MALLOC_CAP_SPIRAM);
      if (!rec_buffers[i])
      {
        M5.Log(ESP_LOG_ERROR, "Failed to allocate buffer %d in PSRAM", i);
      }
    }
  }
  else
  {
    M5.Log(ESP_LOG_ERROR, "No PSRAM available, falling back to heap");
    rec_buffers = new int16_t *[NUM_BUFFERS];
    for (int i = 0; i < NUM_BUFFERS; ++i)
    {
      rec_buffers[i] = new int16_t[record_size];
    }
  }
...
{
  const size_t CHUNK_SIZE   = (BLEDevice::getMTU() - 3) / sizeof(int16_t); // ~254
  const TickType_t NOTIFY_DELAY = pdMS_TO_TICKS((CHUNK_SIZE*1000)/record_samplerate);
